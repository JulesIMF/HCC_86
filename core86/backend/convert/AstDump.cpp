...
void putVertex(FILE* file, Node::Vertex* vertex)
{
    ...
    case Type::PgmEnd:
        fprintf(file, "v%p[label = \"%lu:%lu\\nPgmEnd\", shape = \"parallelogram\", fillcolor = \"blue\", style = \"filled, bold\", fontsize = 15];",
            vertex, vertex->row, vertex->column); break;
    case Type::In:
        fprintf(file, "v%p[label = \"%lu:%lu\\In\", shape = \"parallelogram\", fillcolor = \"blue\", style = \"filled, bold\", fontsize = 15];",
                vertex, vertex->row, vertex->column);
        break;

    case Type::Out:
        fprintf(file, "v%p[label = \"%lu:%lu\\Out\", shape = \"parallelogram\", fillcolor = \"blue\", style = \"filled, bold\", fontsize = 15];",
                vertex, vertex->row, vertex->column);
        break;
...
void astDump(Node::Vertex* astTree)
{
    ...
    fprintf(file, "}");
    fclose(file);
    system("dot -Tpng ast.dot -o ast.png > /dev/null");
    //system("rm ast.dot");
